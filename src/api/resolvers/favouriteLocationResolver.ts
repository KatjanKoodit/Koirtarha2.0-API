...
export default {
  Query: {
    ...
    favouritesByUser: async (_: undefined, args: {userId: string}) => {
      return await favouriteLocationModel.find({user_id: args.userId});
    },
    favouriteById: async (_: undefined, args: {id: string}) => {
      return await favouriteLocationModel.findById(args.id);
    },
  },
  Mutation: {
    addFavourite: async (
